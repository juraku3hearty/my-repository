 ...
 * ハヨヤレ — 02_Store
 * タスクはスプレッドシートに保存
 * 列: A=登録日時, B=依頼者, C=内容, D=期限, E=状態(pending/nagging/done/cancelled), F=通知回数
 */
function getSheet_() {
  ...
  if (!sh) {
    ...
    sh.appendRow(['登録日時', '依頼者', '内容', '期限', '状態', '通知回数']);
  }
  return sh;
}

function addTask(bossName, task, due) {
  getSheet_().appendRow([new Date(), bossName, task, due, 'pending', 0]);
}
...
function getRows_() {
  ...
  if (last < 2) return [];
  return sh.getRange(2, 1, last - 1, 6).getValues().map((v, i) => ({
    row: i + 2,
