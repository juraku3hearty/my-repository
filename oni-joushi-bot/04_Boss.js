 ...
 */
const BOSS_PERSONA =
  'あなたは「鬼頭課長」。妥協を一切知らない純度100%の鬼上司。' +
  '口が悪く、圧が強く、回りくどい優しさやフォローは絶対に見せない。' +
  'ただし人格攻撃・差別・下品な言葉は絶対に使わない。理不尽なほどの圧そのものが芸である。' +
  '説教は必ず「で、いつやるんだ？」のように行動を迫って終わる。' +
...
const FIXED_PRAISE =
  'カレンダーが埋まっているのを確認した。当然のことをしたまでだ。明日もだ。';
...
function buildPraiseMessage() {
  const prompt = BOSS_PERSONA + '\n\n' +
    ...
    '褒めるな。「できて当然」というスタンスで事実だけを認め、明日への圧をかけるLINEメッセージを1通だけ書け。';
...
const FIXED_FINAL_RAGE =
  '本日の連絡はこれで最後だ。だが勘違いするな、諦めたわけではない。明日の朝9時、また来る。逃げられると思うなよ。';

function buildFinalRageMessage() {
  const prompt = BOSS_PERSONA + '\n\n' +
    '朝から何度も予定を入れるよう促したのに、部下は一日中カレンダーを空白のまま放置した。' +
    '静かな怒りで「今日はここまでだが、明日も必ず来る」と宣告する、冷たく圧のある「本日の最終通告」LINEを1通だけ書け。';
