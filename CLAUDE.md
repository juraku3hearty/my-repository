...
## Link Hokkaido — 店舗ページ／LP制作（2026-09 時点）

雛形と作り方の知見は `docs/雛形パターン集.md`。LPの実物は `site/lp/` 配下。
プレビューは Cloudflare Pages（`npx wrangler pages deploy . --project-name=<name> --branch=main --commit-dirty=true`、
パスワードゲートは各 `functions/_middleware.js`）。**環境からは *.pages.dev を閲覧できない**ので、確認は人間側。

### 呼び分け（混同注意：どれもピラティスが絡む）
| 呼び方 | 正式 | 場所 | 立場 |
|---|---|---|---|
| アーマー | ARMOUR GYM pilates studio | 千歳 | Link Hokkaido の顧客（本契約済み） |
| 分院 | いつき整体院（分院・2026-08移転） | 京都 | 自店。ピラティス整体あり |
| 本院 | いつき鍼灸整骨院 | 京都 | 自店。ピラティスなし・緑クリア |

### アーマー — `site/lp/armour-gym/`（PW: armour2026）
- 型：C型ベースだが**体験を絞るページ**（体験に手を取られると既存会員に影響するため、本気の人だけ来てほしい）
- 決定事項：体験は値引きしない／体験も通常と同じ内容でその人に合わせて組む／グループ無し・全マンツーマン／
  セルフ＝予約した時間内に自分でマシンを使う（指導なし・スタッフ在中）／ジムは別料金オプション／
  駐車場＝千歳市グリーンベルト地下駐車場（提携・4時間無料・徒歩約5分）／靴不要・滑り止め靴下必要（店内販売あり）／
  ロッカールーム（鍵付き）・シャワーあり／支払い 現金・カード
- 受付状況は `<body data-status="open|wait">` で切替。**実際に待ちの時だけ wait にする**
- デザイン：競合 ON WAY（明朝・ベージュ・余白）と逆に、黒×写真主役×ゴシック細字。明暗を交互にして光を入れる
- 未確定：体験料金・所要時間／税込税別／定休日／タオル／セルフの予約方法・時間帯／ジム料金／子ども預かり掲載可否／
  インストラクター紹介／お客様の声
- 競合情報：ON WAY（グループ月4回13,000・パーソナル8回52,800）。アーマーは月6回17,000（1回2,833円）で単価優位。**比較表はLPに載せない**

### 分院 — `site/lp/itsuki-bunin/`（PW: itsuki2026）
- 型：F型（引き算）。見出しが否定形で揃う（鍛えるためでは、ありません／聞くのは、2つだけ／治してもらう時間では、ありません）
- 売上データ根拠：ピラティス整体が最も深い商品（30回×4名・10回×6名）。産後は新規の1割強→独立ページにしない
- 決定事項：コース料金は非掲載（初回7,700円のみ）／初回に契約を勧めない（要運用確認）／お客様の声は捏造せず枠のみ
- 美容鍼＋化粧品カウンセリング（何も売らない30分）は `biyoshin.html`
- 未確定：新住所・電話・営業時間・所要時間・担当者名・資格表記・料金確定・写真・声

### 本院 — Cloudflare `itsuki-honin`（PW: itsuki2026）※ローカル元ファイルはコンテナ消失
- 緑クリア／ピラティスなし／手技×EMS、鍼灸はオプション／全員国家資格者
- 未確定：住所・電話・料金・保険/交通事故対応・写真

### 表示調整のメモ（2026-09-09）
- スマホ（600px以下）：本文は**左揃え・改行はそのまま**。中央揃えは見出し・締めの一言・引用・受付状況のみ。
- PC（601px以上）：文中の `<br>` を非表示にして行を長く。900px以上では2カラム化
  （こんな方に／料金／流れ／FAQ）＋マシン3列で縦の間延びを解消。
- 確認方法：Playwright で 390px と 1280px を実描画（`/opt/pw-browsers/chromium_headless_shell-1194/chrome-linux/headless_shell`
  を executablePath に指定。`chromium.launch()` の既定パスは合わない）。
- **未対応：スマホ表示にまだ違和感があるとの指摘（2026-09-09時点）。** 次セッションで実機スクショをもらって調整する。
- 検討中：初回のみネット予約。案①Googleフォーム（希望日時3候補＋気になっていること）が有力。
  先に「アーマーが使っている予約システムに公開ページがあるか」を確認すること。あれば直リンクで済む。

### 書くときの注意（共通）
- 鍼灸は広告制限あり：効果効能の断定・ビフォーアフター・最上級表現を使わない。「続けている方が多い」等の事実で書く
- モニター等で口コミ投稿を条件にするなら、投稿者本文に「モニターとして参加」の明記が必要（ステマ規制）。任意が安全
- 書いたキャッチは**通っている人に事実確認**する（分院で「鍛えません。痩せません。」が両方事実でなかった）
