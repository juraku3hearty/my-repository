/**
 * オンラインサロン自動化スクリプト (Phase 4: Zoom案内を会員ページへ直書き版)
 *
 * Phase 4 変更点:
 *  - updateTopPageZoomLink() を新設：次回ミーティングのZoom案内を、iframe(埋め込み)ではなく
 *    会員トップページの本文へ“実テキスト”として自動で書き込む。
 *    → iPhone(Safari)のサードパーティCookieブロックで消える問題を根本解決。
 *      会員ログインの内側なので退会者は見れず、LINEにも載らない。
 *  - buildZoomBoxHtml_() を新設：doGetと同じ判定ロジックで案内HTMLを組み立てる。
 *  - processOnlineSalonAutomation() に updateTopPageZoomLink() を追加。
 *
 * Phase 3 変更点（履歴）:
 *  - processReadyVideos() … Vimeoのreadyタグ動画を自動でCyfons投稿＋LINE配信
 *  - detectCategory() … 動画タイトルからカテゴリ自動判定
 */
...
function processOnlineSalonAutomation() {
  ...
  updateTopPageZoomLink();   // ← Phase4追加：Zoom案内をページに直書き（iframe廃止）
...
function processReadyVideos() {
  ...
  for (const v of readyVideos) {
    ...
    // readyタグの自動投稿は、サイト掲載＋会員へのLINE一斉配信まで自動（第3引数 true）。
...
/**
 * 【Phase4 新設】会員トップページに「次回のZoom案内」を“直書き”で表示する。
 * iframe(埋め込み)をやめて実テキストで書くので、iPhone(Safari)でも全員が見れる。
 * 退会者は会員サイトに入れないので見れず、LINEにも載らない。
 *
 * 仕組み：
 *  - マーカー <!--ZOOM_AUTO_START--> 〜 <!--ZOOM_AUTO_END--> の間を毎回まるごと入れ替える
 *    （何度実行しても多重表示にならない）。
 *  - 古いGAS埋め込みiframe（script.google.com を指すもの）は自動で撤去する。
 *  - 前日18:30以降＆Zoom URL確定でURL表示。まだならは日付案内。予定が無ければ非表示。
 *  - 開催が終われば次の予定に自動で切り替わる（getEventsが過去分を返さないため）。
 */
function updateTopPageZoomLink() {
  const cookieHeader = getCyfonsCookieHeader();
  if (!cookieHeader) { console.error("Zoom案内更新: Cyfonsログイン失敗（CYFONS_ID/PWを確認）"); return; }
  ...
  // 現在の本文から「前回の自動ブロック」と「古いGAS埋め込みiframe」を除去する
  let contents = match[1];
  contents = contents.replace(/<!--ZOOM_AUTO_START-->[\s\S]*?<!--ZOOM_AUTO_END-->/g, "");
  contents = contents.replace(/<iframe\b[^>]*script\.google\.com[\s\S]*?<\/iframe>/gi, "");

  // doGetと同じ判定で案内HTMLを作る。予定が無ければ null。
  const box = buildZoomBoxHtml_();
  const block = box ? `<!--ZOOM_AUTO_START-->${box}<!--ZOOM_AUTO_END-->` : "";

  // 案内はページ先頭に出す（一番目立つ位置）
  const updatedContents = block + contents;
  ...
  const updatePayload = {
    ...
    keyword: "トップページキーワード",
    contents: updatedContents,
    add_br: "0", public: "1", id: "1", url: "index",
    status: "edit_done", layout: "top"
  };
  const resp = UrlFetchApp.fetch(topPageUrl, { method: "post", payload: updatePayload, headers: cookieHeader, muteHttpExceptions: true });
  console.log("Zoom案内 直書き更新: HTTP=" + resp.getResponseCode() + (box ? " / 表示あり" : " / 予定なし(非表示)"));
}

/**
 * 【Phase4 新設】次回ミーティングの案内HTMLを返す（doGetの表示ロジックと同じ）。
 * 予定が無ければ null。
 */
function buildZoomBoxHtml_() {
  const calendar = CalendarApp.getDefaultCalendar();
  const now = new Date();
  ...
  if (now >= switchTime && zoomUrl.startsWith("https://")) {
    inner = '<p style="margin:0;font-size:0.85em;font-weight:bold;color:#333;">次回のZoom URL</p>'
          + '<div style="margin-top:8px;"><a href="' + zoomUrl + '" style="color:#0369a1;text-decoration:underline;font-weight:bold;font-size:1.1em;">'
          + nextEvent.getTitle() + ' に参加する</a></div>';
  } else {
    inner = '<p style="margin:0;font-size:0.85em;font-weight:bold;color:#333;">次回開催予定</p>'
          + '<div style="margin-top:8px;font-size:1em;color:#1e40af;font-weight:bold;">' + eventDateStr + '　' + nextEvent.getTitle() + '</div>';
  }
  return '<div style="padding:15px;background:#f0f9ff;border:1px solid #7dd3fc;border-radius:8px;margin-bottom:15px;">' + inner + '</div>';
}
...
/**
 * 手動診断用：Zoom案内の直書きが正しく動くか単体で確認する（投稿はしない）。
 * GASエディタでこの関数を選んで実行 → 実行ログ＆会員トップページを確認。
 */
function testZoomDirectWrite() {
  const box = buildZoomBoxHtml_();
  console.log("生成される案内HTML:\n" + (box || "(次回予定なし → 非表示)"));
  updateTopPageZoomLink();
  console.log("→ 会員トップページ https://famitect.biz/members/ を開いて、案内が本文に出ているか確認してください。");
}
...
function checkReadyVideos() {
  ...
  (vData.data || []).forEach(v => {
    const vid = v.uri.split("/").pop();
    const tagNames = (v.tags || []).map(t => t.name);
    if (tagNames.some(t => t === "ready")) {
      found++;
      console.log(`ready動画: ${v.name} / id=${vid} / 処理済み=${isAlreadyProcessed(vid)} / tags=[${tagNames.join(", ")}]`);
    }
  });
  if (found === 0) console.log("⚠️ readyタグ付きの動画が1本もありません");
}
