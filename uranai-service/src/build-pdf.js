...
function bodyContent(astro) {
  ...
  const kanT = sec('官祿', 'あなたが力を発揮しやすいのは、こんな場です。', D.KAN, '人前で持ち味を活かすほど、信頼や評価につながります。');
  const fuuT = sec('夫妻', '人との関わりでは、こんな傾向があります。', D.FUU, '気持ちを溜め込まず、短くていいから言葉にしてみると、ご縁が深まります。');
  const zaiT = sec('財帛', 'お金とは、こんな付き合い方が向いています。', D.ZAI, '');
  const fukT = sec('福德', '心がいちばん満たされるのは、こんな時間です。', D.FUK, '');
  const closing = 'ここに書いたのは、あなたが生まれ持った「傾向」です。当たっているところは活かし、ピンとこないところは横に置いて大丈夫。大切なのは、自分を責めずに、持ち味を活かす方へ少しずつ舵を切ること。あなたが、あなたの人生の主役です。';

  return [
    { type: 'h', t: 'あなたの中心にあるもの（命宮）' }, ...center.map((p) => ({ type: 'p', t: p })),
    ...cmut.map((p) => ({ type: 'p', t: p })), ...shin.map((p) => ({ type: 'p', t: p })),
    { type: 'h', t: 'あなたの強み' }, { type: 'p', t: 'あなたの持ち味を、言葉にするとこんな感じです。' }, { type: 'ul', items: cho },
    { type: 'h', t: '気をつけたいクセ' }, { type: 'p', t: 'これは欠点ではなく、強みが少し行きすぎたときに出るクセです。知っておくと、ぐっと扱いやすくなります。' }, { type: 'ul', items: tan },
