...
function reader(astro) {
  ...
  // 遷移は辞書が「外で好かれる」など近い表現で重複しがち＝2つ並べると日本語が冗長。輝き優先で一番明るい星のみ（方向は下の離郷/外向きの結論が担う）。
  let sp = `外の世界：${(senLead && D.SEN[senLead]) || '外でも自然体で過ごせるタイプ'}。`;
  ...
  else sp += '住み慣れた場所や、勝手のわかる環境にいるほど、落ち着いて力を発揮できるタイプ。あちこち動き回るより、腰を据えてじっくり取り組むほうが、あなたらしさが活きます。';
  blocks.push(Pp(sp));
...
const JOTEI_INTRO = [
  { type: 'p', t: 'そなたの命盤を、ここに記す。' },
  { type: 'p', t: 'その星々は、生まれ持った才と宿命を映し出す鏡なり。' },
  { type: 'p', t: '己を知ることは、未来を知ることに等しい。' },
  { type: 'p', t: 'ゆえに心を静め、この書を開くがよい。' },
  { type: 'p', t: 'ここより先は、そなたという人間の物語である。' },
];
